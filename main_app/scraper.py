...
def create_models():
    ...
    if Global.objects.all().exists():
        ...
        g.china_recovered=int(page_dict['summaryStats']['china']['recovered'])
        g.china_deaths=int(page_dict['summaryStats']['china']['deaths'])
        g.nonchina_infected=int(page_dict['summaryStats']['nonChina']['confirmed'])
        g.nonchina_recovered=int(page_dict['summaryStats']['nonChina']['recovered'])
        g.nonchina_deaths=int(page_dict['summaryStats']['nonChina']['deaths'])
        g.save()
    else:
        ...
        china_infected=int(page_dict['summaryStats']['china']['confirmed']),
        china_recovered=int(page_dict['summaryStats']['china']['recovered']),
        china_deaths=int(page_dict['summaryStats']['china']['deaths']),
        nonchina_infected=int(page_dict['summaryStats']['nonChina']['confirmed']),
        nonchina_recovered=int(page_dict['summaryStats']['nonChina']['recovered']),
        nonchina_deaths=int(page_dict['summaryStats']['nonChina']['deaths']),
        )
        global_object.save()
    ...
    for all_sets in page_dict['rawData']:
        if page_dict['rawData'][num]['Province/State'] == '':
            if page_dict['rawData'][num]['Country/Region'] in Country.objects.all().values_list('name', flat = True):
                c = Country.objects.get(name=page_dict['rawData'][num]['Country/Region'])
            ...
            if page_dict['rawData'][num]['Province/State'] in Province.objects.all().values_list('name', flat=True):
                p = Province.objects.get(name=page_dict['rawData'][num]['Province/State'])
                p.name=page_dict['rawData'][num]['Province/State']
                p.country=Country.objects.get(name=page_dict['rawData'][num]['Country/Region'])
                ...
                if 'Princess' in page_dict['rawData'][num]['Province/State'] or page_dict['rawData'][num]['Province/State'] == 'US':
                    ...
                    province_object = Province(
                        ...
                        country=Country.objects.get(name=page_dict['rawData'][num]['Country/Region']),
                        infected=int(page_dict['rawData'][num]['Confirmed']),
                        recovered=int(page_dict['rawData'][num]['Recovered']),
                        deaths=int(page_dict['rawData'][num]['Deaths']),
                    )
